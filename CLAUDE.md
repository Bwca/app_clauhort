...
`@mentions` are parsed by `server/services/messageRouter.js`: `parseResponders` decides who responds to a user message (broadcast to non-observers if nothing's mentioned), `extractMentionedAgents` is used both for that and for the **relay** mechanism (an agent's own reply `@mentioning` a teammate triggers that teammate), and `parseSkillInvocation` detects a message that is *entirely* `@Name /command ...` to invoke one of that agent's real Claude Code skills (listed via `server/services/commands.js`, which reads `.claude/commands/*.md` straight off the agent's `workingDir`). The relay's trigger content is always the mentioning teammate's own reply, never the original user message or a stripped skill command. By default a chat is **bounded**: relay stops after exactly one hop, and never fires back to an agent who already responded in the same chain — this is what stops even a two-agent mutual-mention from starting a loop. A chat's `freeRelay` flag (toggled per-chat via the 🔁 button in the topbar, `PATCH /api/chats/:id`) lifts that: rounds keep going, the same agent can be relayed again in a later round, for as long as each round produces new @mentions — bounded only by `FREE_RELAY_MAX_ROUNDS` in `ws/handler.js` as a safety ceiling against a genuinely unbounded two-agent back-and-forth.
