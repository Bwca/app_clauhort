 ...
 * ...args", nothing else, AND "/command" is one of that agent's actual
 * registered custom commands (`.claude/commands/*.md` in its workingDir —
 * see commands.js). Returns null for anything else (normal chat), including
 * a mention followed by ordinary text that merely contains a "/", AND a
 * syntactically-valid "@Name /word" whose word isn't a real command.
 *
 ...
 * larger set than this project's custom commands) by that same bare-leading-
 * slash syntax. Confirmed live: sending "@SomeAgent /chrome" against an
 * agent with no "/chrome" custom command got the CLI's own local-command
 * dispatcher to intercept it — {"type":"system","subtype":"local_command"}
 * in the session transcript — before the model ever saw the turn, silently
 * discarding the [System] preamble sent alongside it in the same turn (see
 * buildPromptBlocks in ws/handler.js). Since that preamble only ever goes
 * out once per agent per chat, the agent's identity was lost for good the
 * moment its first-ever message happened to collide with a CLI-native
 * command name. Validating against the real command list closes this:
 * anything that isn't an actual registered skill now falls through to
 * parseResponders as ordinary chat text (content kept intact, "@Name "
 * prefix included), which doesn't match the CLI's bare-slash trigger.
 *
 * @param {string} content - Raw message content from the user
 * @param {import('../store/db.js').Agent[]} chatMembers - Agents currently in the chat
