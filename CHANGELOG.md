...
### Added
- New ⚡ "Quick messages" button next to the composer lets you save short message snippets you send often (e.g. "Please continue") and insert one at the cursor with a click instead of retyping it — never auto-sent, so there's always a chance to review or tweak it first. Manage the list (add/edit/delete) right from the same dropdown. Global across every chat, persisted in a new `quick_messages` table. See `server/store/db.js`, `server/routes/quickMessages.js`, and `renderQuickMsgPanel`/`useQuickMessage` in `server/public/app.js`.
- Agents can now have their model switched — at creation via the new Model field, or on an existing agent via the ⋮ menu's "Switch model" action — accepting the same values the `claude` CLI's own `--model` flag does (an alias like "opus"/"sonnet"/"fable", or a full model ID). Confirmed empirically that pairing `--model` with `--resume` genuinely switches an existing session's model without losing conversation history (the CLI reports a one-time prompt-cache rebuild on that turn, not a fresh session). Baked into spawn args like `workingDir`/`resumeId`, so changing it evicts and respawns the agent's process the same way. See `modelOverride` in `server/store/db.js`, `buildArgs` in `server/services/agentProcessManager.js`, and `changeAgentModel` in `server/public/app.js`.
- Each agent's sidebar entry now shows which real model it's actually running on (e.g. "🧠 Sonnet 5"), captured from the `claude` CLI's own stream-json output on its most recent turn — this app never requests a specific model itself, so previously it was invisible which one the CLI had resolved. Persisted per agent (`last_known_model` in `server/store/db.js`, `setAgentModel`) and kept live via `AGENT_UPDATED`; see `model` on `TurnAccumulator`/`runTurn` in `server/services/agentProcessManager.js` and `formatModelLabel` in `server/public/app.js`.
- Chats can now be grouped into categories in the sidebar — hover a chat and click the 🏷️ button to set/clear its category (autocompletes against existing ones); each category renders as a collapsible section, with Uncategorized always first. A category's ✏️/× controls rename or clear it across every chat that has it in one step, since a category is just a shared label rather than its own record. See `category` on `Chat`/`updateChat`/`renameChatCategory` in `server/store/db.js`, `PATCH /api/chats/:id` and the new `PATCH /api/chats/category` in `server/routes/chats.js`, and `renderChatList`/`startCategoryEdit`/`renameCategory`/`deleteCategory` in `server/public/app.js`.
- Every rendered code block in an agent's reply now gets a copy button (revealed on hover, top-right of the block) — no more manually selecting code to copy it. See `decorateCodeBlocks`/`copyCodeBlock` in `server/public/app.js` and the `.md-code-wrap` container `markdown.js`'s `renderer.code` now wraps each block in.
